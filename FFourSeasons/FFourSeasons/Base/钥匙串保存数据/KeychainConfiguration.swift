...
struct KeychainConfiguration {
    static let serviceName = "My1A2p4p5Se&rvice"
    ...
    static func save(userName:String,passwork:String,forKey:String) ->Void{
        do{
            UserDefaults.standard.set(userName, forKey: forKey)
            ...
            try passwordItem.savePassword(passwork)
        }
        catch {
    ...
    static func get(forKey:String) ->KeychainPasswordItem?{
        let accountName = UserDefaults.standard.string(forKey: forKey)
        if accountName != nil {
