...
- (UITableView *)baseSearchTableView
{
    if (!_baseSearchTableView) {
        UITableView *baseSearchTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 64+_searchTitleView.py_y+_searchTitleView.py_height, self.view.py_width, self.view.py_height-(_searchTitleView.py_y+_searchTitleView.py_height+10)) style:UITableViewStyleGrouped];
...
- (PYSearchSuggestionViewController *)searchSuggestionVC
{
    if (!_searchSuggestionVC) {
        ...
        searchSuggestionVC.view.frame = CGRectMake(0, 64+_searchTitleView.py_y+_searchTitleView.py_height+10, self.view.py_width, self.view.py_height-(_searchTitleView.py_y+_searchTitleView.py_height+10));
...
- (void)setup
{
    ...
    //创建 segmentControl 分类
    [self.view addSubview: [self setupSegmentControl:@[@"宝贝",@"商店"]]];
    
    // 创建搜索框
    _searchTitleView = [[UIView alloc] init];
    _searchTitleView.py_x = PYSEARCH_MARGIN * 2;
    _searchTitleView.py_y = 120;
    _searchTitleView.py_width = self.view.py_width - _searchTitleView.py_x * 2;
    _searchTitleView.py_height = 30;
    UISearchBar *searchBar = [[UISearchBar alloc] initWithFrame:_searchTitleView.bounds];
    
    [_searchTitleView addSubview:searchBar];
    _searchTitleView.layer.borderWidth = 1;
    _searchTitleView.layer.borderColor = [UIColor lightGrayColor].CGColor;
    [self.view addSubview:_searchTitleView];
    // 关闭自动调整
    searchBar.translatesAutoresizingMaskIntoConstraints = NO;
    // 为titleView添加约束来调整搜索框
    NSLayoutConstraint *widthCons = [NSLayoutConstraint constraintWithItem:searchBar attribute:NSLayoutAttributeWidth  relatedBy:NSLayoutRelationEqual toItem:_searchTitleView attribute:NSLayoutAttributeWidth multiplier:1.0 constant:0];
    NSLayoutConstraint *heightCons = [NSLayoutConstraint constraintWithItem:searchBar attribute:NSLayoutAttributeHeight  relatedBy:NSLayoutRelationEqual toItem:_searchTitleView attribute:NSLayoutAttributeHeight multiplier:1.0 constant:0];
    NSLayoutConstraint *xCons = [NSLayoutConstraint constraintWithItem:searchBar attribute:NSLayoutAttributeTop  relatedBy:NSLayoutRelationEqual toItem:_searchTitleView attribute:NSLayoutAttributeTop multiplier:1.0 constant:0];
    NSLayoutConstraint *yCons = [NSLayoutConstraint constraintWithItem:searchBar attribute:NSLayoutAttributeLeft  relatedBy:NSLayoutRelationEqual toItem:_searchTitleView attribute:NSLayoutAttributeLeft multiplier:1.0 constant:0];
    [_searchTitleView addConstraint:widthCons];
    [_searchTitleView addConstraint:heightCons];
    [_searchTitleView addConstraint:xCons];
    [_searchTitleView addConstraint:yCons];
    ...
    contentView.py_x = PYSEARCH_MARGIN * 2;
...
- (UILabel *)setupTitleLabel:(NSString *)title
{
    ...
    titleLabel.py_x = 0;
    titleLabel.py_y = _searchTitleView.py_height+64;
    return titleLabel;
}

-(UISegmentedControl *) setupSegmentControl:(NSArray *)items{
    UISegmentedControl * segmentControl = [[UISegmentedControl alloc] initWithItems:items];
    segmentControl.frame = CGRectMake(PYSEARCH_MARGIN * 2,91, 100, 30);
...
-(void)change:(UISegmentedControl *)sender{
    self.selectBtnIndex = sender.selectedSegmentIndex;
}
